...
<div class="p-4 w-1/2 mx-auto rounded-xl shadow-xl bg-gray-100">
    <form class="max-w-sm mx-auto" method="POST" action="{{route('articles.store')}}">
        @csrf
        <div class="mb-5">
        ...
        <div class="mb-5">
            ...
            <input type="radio" @checked(@old('disponible') == 'SI') name="disponible" value="SI" class="mr-2" />Sí &nbsp;&nbsp;
            <input type="radio" @checked(@old('disponible') == 'NO') name="disponible" value="NO" class="mr-2" />No
            @error('disponible')
            <x-error>{{$message}}</x-error>
            @enderror
        </div>
